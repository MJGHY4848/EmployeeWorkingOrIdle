...
def detect_hand_motion(prev, curr, threshold=15):
    try:
        return any(np.linalg.norm(np.array(curr[i][:2]) - np.array(prev[i][:2])) > threshold for i in [9, 10])
    except:
        return False
...
while cap.isOpened():
    ...
    for idx, (kp_tensor, box) in enumerate(zip(results.keypoints.xy, results.boxes)):
        ...
        label = "Sitting - Working" if posture == "Sitting" else (
                "Standing - Working" if motion and posture == "Standing" else
                "Standing - Not Working" if posture == "Standing" else
                "Sitting - Not Working")
        color = (0, 255, 0) if "Working" in label else (0, 0, 255)
        ...
        if "Working" in label and posture == "Sitting":
            ...
            tracker["work"] += now - tracker["start"]
            log_work(pid, now - tracker["work"], now, tracker["work"])
            tracker["start"], tracker["log"] = None, now       #We removed tracker["work"] =0
